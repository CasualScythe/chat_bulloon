...
// 3. 서버로부터 'user joined' (유저 접속) 이벤트를 받으면
...
socket.on('user joined', function (userData) {
  ...
  // 만약 해당 유저의 메시지 영역이 없다면 (새로운 접속자)
  if (!userMessageDiv) {
    // 새로운 유저 메시지 영역 (div) 생성
    userMessageDiv = document.createElement('div');
    userMessageDiv.classList.add('userMessage'); // CSS 클래스 추가
    userMessageDiv.setAttribute('data-socket-id', userData.id); // socket.id를 data 속성으로 저장 (나중에 찾기 위함)

    // 유저 닉네임을 표시할 h2 요소 생성
    const nicknameElement = document.createElement('h2');
    nicknameElement.textContent = userData.nickname; // 닉네임 설정

    // 초기 메시지(접속했습니다!)를 표시할 p 요소 생성
    const messageElement = document.createElement('p');
    messageElement.textContent = '접속했습니다!'; // 초기 메시지 내용
    ...
    // 이미 유저 영역이 있는 경우 (예: 페이지 새로고침 후 재접속 등)
    // 메시지와 닉네임만 업데이트 해줄 수도 있음 (여기서는 간단히 처리)
  ...
  logItem.textContent = `${userData.nickname} 님이 접속했습니다.`; // 로그 메시지 내용 설정
...
// 4. 서버로부터 'user left' (유저 퇴장) 이벤트를 받으면
...
socket.on('user left', function (userData) {
  ...
  const logItem = document.createElement('li');
  logItem.textContent = `${userData.nickname} 님이 퇴장했습니다.`; // 로그 메시지 내용 설정
  logItem.classList.add('system-message'); // 시스템 메시지임을 나타내는 CSS 클래스 추가
  logMessages.appendChild(logItem); // 로그 목록 (ul)에 메시지 추가
...
// 5. 서버로부터 'chat message' (채팅 메시지) 이벤트를 받으면
...
socket.on('chat message', function (data) {
  ...
  // 유저별 메시지 영역에 표시할 글자수 제한 설정 (원하는 숫자로 바꿔봐!)
  const charLimit = 80;
  ...
  if (userMessageDiv) {
    ...
    userMessageDiv.querySelector('p').textContent = displayMessage; // ***자른 메시지로 업데이트***

    // (선택 사항) 해당 유저 영역에 메시지 도착 시 강조 효과나 애니메이션 추가 가능
  } else {
    // (일반적으로는 user joined 이벤트가 먼저 오므로 이 경우는 거의 없음)
    // 만약 해당 유저 영역이 없다면 경고 로그 출력
    console.warn(`메시지를 보냈지만 유저 영역이 없는 socket.id: ${data.id}. user joined 이벤트가 누락되었을 수 있습니다.`);
    // 이 else 문에서는 별도의 DOM 생성 로직을 넣지 않음. user joined에서 처리되도록 함.
  }

  // 2) #chatLog 영역에 실시간 메시지 추가
  // 로그에는 ***전체 메시지 내용***을 그대로 추가함!
  const logItem = document.createElement('li');
...
// 페이지가 로드되면 닉네임 입력 필드에 포커스
window.addEventListener('load', function () {
  nicknameInput.focus(); // 닉네임 입력 필드에 자동으로 포커스
});